...
public class ProductPage implements Initializable ,CreatingCartOfCostumerAndUpdateDatabase , LoginChecker,BackToMainPage,SearchInProducts{
    ...
    public static void recieveMessage(String messageFromServer, VBox vBox) {
        ...
        hBox.setPadding(new Insets(5, 25, 5, 10));
        ...
        textFlow.setStyle(
                ...
                "-fx-font-family: 'IRANYekanWeb Medium' ;" +
                "-fx-font-size: 14px;" +
                "-fx-text-fill: white");
    ...
    public void sendComment(ActionEvent event){
            if (MainPage.customer != null) {
                ...
                if (!messageToSend.isBlank()) {
                    ...
                    connection = database.connectDB();
                    boolean isBought = history.contains(String.valueOf(product.getProductId()));
                    try {
                        statement = connection.createStatement();
                        statement.executeUpdate("INSERT INTO comments (Text,productid,Username,isbought) VALUES ('" + messageToSend + "','" + product.getProductId() + "','" + MainPage.customer.getUsername() + "','"+isBought+"')");
                    } catch (SQLException e) {
        ...
        try {
            ...
            while (resultSet.next()){
                ...
                if (resultSet.getString("productid").equals(String.valueOf(product.getProductId()))) {
                    if (isBought.equals("true")) {
                        recieveMessage(resultSet.getString("Username") + " (has bought) : " + resultSet.getString("Text"), CommentVBox);
                        System.out.println(resultSet.getString("Username") + " : " + text);
                    }
                    else {
                        recieveMessage(resultSet.getString("Username") + " (has not bought) : " + resultSet.getString("Text"), CommentVBox);
                        System.out.println(resultSet.getString("Username") + " : " + text);
                    }
                }
            }
    ...
    private void vote(int pointDatabase){
        if (history.contains(String.valueOf(product.getProductId()))) {
            connection = database.connectDB();
            try {
                statement = connection.createStatement();
            ...
            alert.setTitle("Error Message");
            alert.setHeaderText(null);
            alert.setContentText("you have not purchased this product");
            alert.showAndWait();
        }
    }
    public void point1() {
...
//        }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        showComments();
        if (isLogin()) {
            Set<String> set = readHistory().keySet();
            history.addAll(set);
            for (String s :
                    history) {
